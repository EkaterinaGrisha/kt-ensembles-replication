...
def process_cell(dataset, fold, granularity: str = "concept"):
    try:
...
def main():
    ...
    if args.out is None:
        ...
        args.out = paths.ARTIFACTS_DIR / "ensembles" / f"attention_gating_deep{suffix}.csv"
    args.out.parent.mkdir(parents=True, exist_ok=True)

    rows = []
    for ds in args.datasets:
        for fold in args.folds:
            r = process_cell(ds, fold, args.granularity)
            if r is None:
